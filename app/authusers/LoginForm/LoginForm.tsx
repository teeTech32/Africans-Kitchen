...
export default function LoginForm(){
  ...
  const [state, formAction, isPending] = useActionState(loginUsers, initialState)
...
}, [state.message, state.information, router, callbackUrl]);
  ...
  function handleChange(event: ChangeEvent<HTMLInputElement>){
   ...
   data-aos-easing='linear'>
              {loginAlert && <p className={`${loginAlert === state.information  ? 'bg-green-600' : 'bg-red-600'}  p-2 mx-10 md:mx-20 rounded-md text-white text-center text-sm md:text-lg xl:text-xl font-bold my-15 md:mt-10`}>
                 {loginAlert}
               </p>}
                        ...
                        <button type="button" className="absolute top-2 right-2 ">
                         ...
                         <form action={formAction} >
                           ...
                           <div className="mx-5 mt-5 mb-3 flex flex-col">
                             <label htmlFor="Password" className="text-white text-xs font-semibold mb-0">PASSWORD</label>
                           ...
                           <p className="float-right ">
                              <button  type="button"  className='text-white font-extrabold text-xs  hover:text-yellow-400 cursor-pointer hover:transition-1000 duration-500 hover:scale-105 mx-5'onClick={()=>router.push('/authusers/CheckEmail')}>     
                                Forgot Password
                              </button>
                           </p>
                           <p className="text-center  mt-15 mb-3">
                             <button  type="submit" disabled={isPending} className='w-32 text-black font-extrabold text-xs bg-gradient-to-l from-red-700 via-yellow-400 to-orange-400 p-2 rounded-md cursor-pointer hover:text-white hover:transition-1000 duration-500 hover:scale-105'>{isPending ? 'Loging...' : 'LogIn'}</button>
                           </p>
                           <div className="flex justify-center mt-5">
                             <button type="button" className="text-center text-white text-xs"> Haven&apos;t you created an account ?<span className='text-black font-extrabold rounded-md cursor-pointer  hover:text-white hover:transition-1000 duration-500 hover:scale-105 ml-1' onClick={()=>router.push('/authusers/SignUp')}>SignUp</span>
                             </button>
                           </div>
                         </form> 
                       </div>
