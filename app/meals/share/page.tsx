...
import { startTransition, useEffect, useActionState, useState, type ChangeEvent } from "react"
...
export default function ShareMeals(){
  ...
  const {visibleMessage, setVisibleMessage, loading, pickedImage, setPickedImage, user, fetchUser } = useFormContext();
  const initialState: ShareMealState = {
    message: null,
    error: null
  }
  const [state, formAction, isPending] = useActionState(sharemealData, initialState);
  const [spinner, setSpinner] = useState(false)
  const [lastFormData, setLastFormData] = useState(null);
  const [notification, setNotification] = useState(false);
  const [zodInputValidation, setZodInputValidation] = useState(false);
  const [zoderrormessage, setZoderrormessage] = useState<string | null>(null)
  ...
  async function handleAiRequest(){
    try{
      ...
      setZoderrormessage(err.response?.data?.error)
  ...
  const handledSubmitForm = (formData: any)=>{
  ...
  function handleChange(event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>){
    setFormInputData({...formInputData, [event.target.id]: event.target.value})
  }
          ...
          <form action={handledSubmitForm} >
            <div className="flex flex-col md:flex-row">
              <p className="mx-5  my-2">
                <label htmlFor="userName" className="text-gray-400 text-xs md:text-sm xl:text-lg font-semibold">YOUR NAME</label>
              ...
              <p className="mx-5 my-2">
                <label htmlFor="userEmail" className="text-gray-400 text-xs md:text-sm xl:text-lg font-semibold">YOUR EMAIL</label>
            ...
            <ImagePicker label={'Preview your image'} name={'image'} isRequired={state.message ?? null} pickedImage={pickedImage} setPickedImage={setPickedImage}/>
