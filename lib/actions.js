...
import { verifyAccessToken } from "@/app/api/utils/tokens/route";
...
export async function sendEmail(prevState, formData){
  ...
  function isValidEmailInput(input){
    return !input || input.trim() === ''
  }
  if(isValidEmailInput(fullname) || 
     isValidEmailInput(email) || 
     isValidEmailInput(phonenumber) || 
     isValidEmailInput(subject) || 
     isValidEmailInput(message) || 
     !email.includes("@")){
      return {error: "Invalid Inputs, check your inputs." }
     }
  const emailData = {
    fullname: fullname,
    email: email,
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/utils/sendmail`, {
    method: "POST",
    headers: {"Content-Type" : "application/json"},
  ...
  const data = await response.json();
  if(data?.error){
    return {error: data.error || "Check your internet connection, if your email is valid"}
  }
  return {message: data.message || "Your email is sent successfully"}
}
