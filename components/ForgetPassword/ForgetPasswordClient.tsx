...
export default function ForgetPasswordClient(){
  ...
  const [passwordReclaimAlert, setPasswordReclaimAlert] = useState<string>('')
  ...
  async function handleOnsubmit(e: SubmitEvent<HTMLFormElement>){
    try{
      e.preventDefault()
      setIsPending(true)
      ...
      if(!token){
        setPasswordReclaimAlert("Invalid or Expired token");
        setIsPending(false)
        return;
      }
