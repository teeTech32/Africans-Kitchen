...
export default function HomeClient(){
  ...
  const[notifyAcception, setNotifyAcception] = useState<boolean>(false);
  const[notifyRejection, setNotifyRejection] =  useState<boolean>(false);
          ...
          <main className="text-white px-6 mb-20">
            <section className="my-10">
              ...
              <div data-aos='zoom-in'
                ...
                <p className="flex justify-center text-xs md:text-sm leading-5 mb-2">
                  In order to operate Africans&apos; Kitchen, there are proper webflow procedures you must follow. These procedures are: Register as a user, Login through a onetime email-box-message, Now you have access to: 1. Manually create meal or use AI to generate your meal summary and cooking instructions while you manually pick your meal image. 2. Preview your image before uploading it, but your meal image will be rejected if it&apos;s greater than 3mb. 3. Edit your meal. 4. Delete your meal. 5. View your meal and other poeples&apos; meal details. 6. Reset-your-password through an email-box-message, and 7. You can also get in touch through direct WhatApp message or send an email through the Contact Us nav-button.
                </p>
                <p className="flex justify-center text-xs md:text-sm leading-5">
                  Some features and pages are checked, while some are not. Every registered user has to two different types of token called access-token and refresh-token. These tokens are used to authorize users access to features and pages. The access-token expires within 15minutes and can be refreshed by the refresh-token which expires within 7days or when you logout. The access-token will keep on been refresed if you are not idle for 7days on the web application or if you didn&apos;t logout. 
                </p>
              </div>
            </section>
            <section className="my-10">
            ...
            <div>
              <a href="https://wa.me/2347032603814"  target="_blank"  rel="noopener noreferrer" onMouseOver={()=>setWhatsappNotification(true)} onMouseOut={()=> setWhatsappNotification(false)}>
                <Image src={Whatsapp} alt="WhatsApp" className="w-12 md:w-14 fixed right-5 md:right-12 bottom-20 bg-green-500 rounded-full drop-shadow-2xl z-50 text-base hover:scale-125 duration-300"/>
              </a>
              {whatsappNotification && <p className="bg-black text-white py-2 px-3 border-2 border-green-500 rounded-b-lg rounded-l-lg text-xs md:text-sm fixed right-10 md:right-19.5 bottom-10 md:bottom-8 z-50 ">Chat with us</p>}
            </div>
            <section>
